...
def enhanced_train(low_dir, enh_dir, meta_file, epochs=100, batch_size=8, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (low, enh, cond, mask) in enumerate(train_loader):
            ...
            # Forward pass - FIXED: Pass full condition vector, not just color_shifts
            optimizer.zero_grad()
        ...
        with torch.no_grad():
            for low, enh, cond, mask in val_loader:
                ...
                # FIXED: Pass full condition vector, not just color_shifts
                residual = model(low_adjusted, cond, structure_map)
                output = torch.clamp(low_adjusted + residual * 0.5, 0.0, 1.0)
...
def load_model_for_inference(model_path, device):
    ...
    edge_model.load_state_dict(checkpoint['edge_model_state_dict'])
...
def enhance_image(model, edge_model, image_path, condition, device):
    ...
    cond_tensor = torch.tensor(condition, dtype=torch.float32).unsqueeze(0).to(device)
    
    with torch.no_grad():
        # Apply preprocessing
        brightness = cond_tensor[:, :1]
        color_shifts = cond_tensor[:, 1:]
        
        low_hsv = KC.rgb_to_hsv(img_tensor)
        low_hsv[:, 2:3, :, :] = torch.clamp(
            low_hsv[:, 2:3, :, :] + brightness.view(-1, 1, 1, 1) * 2.0,
            0.0, 1.0
        )
        low_adjusted = KC.hsv_to_rgb(low_hsv)
        low_adjusted = torch.clamp(
            low_adjusted + color_shifts.view(-1, 3, 1, 1),
            0.0, 1.0
        )
        ...
        # Enhancement
        residual = model(low_adjusted, cond_tensor, structure_map)
        output = torch.clamp(low_adjusted + residual * 0.5, 0.0, 1.0)
